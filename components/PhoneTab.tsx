...
export default function PhoneTab({queueIds, onQueueChange}: {queueIds: string[], onQueueChange: (ids:string[])=>void}) {
  ...
  useEffect(() => {
    ...
    setQueue(prev => {
      const existingIds = new Set(Array.from(prev.map(q=>q.leadId)))
      const toAdd = queueIds
    ...
    for (const item of waiting) {
      ...
      try {
        ...
        if (provider==='vapi' && callId) {
          ...
          const outcome: CallOutcome = (['answered','voicemail','no-answer','booked'] as CallOutcome[])[Math.floor(Math.random()*4)]
    ...
    while (callingRef.current && Date.now()-start < maxWait) {
      ...
      try {
        ...
        if (data.status==='ended') {
          const transcript: TranscriptLine[] = (data.artifact?.transcript||[]).map((t: {role:string,message?:string,content?:string})=>({
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'340px 1fr', flex:1, overflow:'hidden'}}>
      ...
      {/* ── Config panel – fully scrollable ── */}
      <div style={{background:'#fff', borderRight:'1px solid #e4e4e0', overflowY:'auto', display:'flex', flexDirection:'column', gap:14, padding:18}}>

        <Section title="Voice AI provider" sub="Select calling platform">
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginBottom:8}}>
            {PROVIDERS.map(p=>(
              <div key={p.id} onClick={()=>setProvider(p.id)} style={{border:`1.5px solid ${provider===p.id?'#2563eb':'#e4e4e0'}`, borderRadius:8, padding:'9px 10px', cursor:'pointer', background:provider===p.id?'#eff6ff':'#fff', transition:'all .12s'}}>
          ...
          <Field label={PROVIDERS.find(p=>p.id===provider)?.name+' API Key'}>
            ...
            <div style={hintSt}>{PROVIDERS.find(p=>p.id===provider)?.hint}</div>
          </Field>
          <Field label="Phone number / Phone ID">
            ...
            <div style={hintSt}>Vapi: use the Phone Number ID from your dashboard (not the raw number)</div>
          </Field>
        </Section>

        <Section title="Call behavior" sub="What the AI does in each scenario">
          <Field label="Your agency name">
            <input value={agencyName} onChange={e=>setAgencyName(e.target.value)} style={inputSt} />
          </Field>
          <Field label="Your name">
            <input value={callerName} onChange={e=>setCallerName(e.target.value)} style={inputSt} />
          </Field>
        ...
        <Section title="Calendly booking" sub="AI books calls directly into your calendar">
          <Field label="Calendly event URL">
            <input
              value={bookingLink}
              onChange={e=>setBookingLink(e.target.value)}
              placeholder="https://calendly.com/evan/seo-audit"
              style={inputSt}
            />
            <div style={hintSt}>The specific event link prospects will use to book</div>
          </Field>
          <Field label="Calendly API token (for auto-booking)">
            <input
              type="password"
              value={calendlyToken}
              onChange={e=>setCalendlyToken(e.target.value)}
              placeholder="Bearer token from Calendly..."
              style={inputSt}
            />
            <div style={hintSt}>
              Get at <a href="https://calendly.com/integrations/api_webhooks" target="_blank" style={{color:'#2563eb'}}>calendly.com/integrations/api_webhooks</a> → Personal Access Token. Enables the AI to confirm booking directly with the prospect.
            </div>
          </Field>
          <div style={{background:'#f0fdf4', border:'1px solid #86efac', borderRadius:8, padding:'9px 11px', fontSize:11, color:'#166534', lineHeight:1.5}}>
            <strong>How auto-booking works:</strong> When a prospect agrees to a call, the AI reads out your next 3 available slots from Calendly, they pick one, and the booking is confirmed in real time — no manual follow-up needed.
          </div>
        </Section>
        ...
        <Section title="Add leads from saved" sub="Pick leads with phone numbers">
          <div style={{display:'flex', flexDirection:'column', gap:4, maxHeight:220, overflowY:'auto'}}>
            {leads.filter(l=>l.phone).length===0 && (
              <div style={{fontSize:12, color:'#9ca3af', padding:'10px 0'}}>No leads with phone numbers yet. Run a prospector search first.</div>
            )}
            {leads.filter(l=>l.phone).map(l=>(
              <div key={l.id} style={{display:'flex', alignItems:'center', gap:8, padding:'6px 8px', borderRadius:7, border:'1px solid #f0f0ec', background:'#fafaf9'}}>
                ...
                <button
                  onClick={()=>addToQueue(l)}
                  disabled={!!queue.find(q=>q.leadId===l.id)}
        ...
        {/* Bottom padding so last section isn't clipped */}
        <div style={{height:8}} />
      </div>

      {/* ── Queue + transcript panel ── */}
      <div style={{display:'grid', gridTemplateColumns:selectedCall ? '1fr 360px' : '1fr', flex:1, overflow:'hidden'}}>
        ...
        <div style={{overflowY:'auto', padding:18, display:'flex', flexDirection:'column', gap:14}}>
          <div style={{display:'flex', alignItems:'center', gap:10, flexWrap:'wrap'}}>
            <div>
              ...
              <div style={{fontSize:11, color:'#9ca3af', marginTop:1}}>AI calls each prospect with a personalized pitch · books directly into Calendly when they agree</div>
            </div>
            <div style={{marginLeft:'auto', display:'flex', gap:8}}>
              <button onClick={()=>setQueue([])} style={btnSt}>Clear queue</button>
              {!calling
                ? <button onClick={startCalls} disabled={!queue.filter(q=>q.status==='queued').length} style={{...btnSt, background:'#16a34a', color:'#fff', border:'none', opacity:queue.filter(q=>q.status==='queued').length?1:.4}}>▶ Start calling</button>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8}}>
            {[
              {n:waiting,    l:'In queue',    c:'#18181b'},
              {n:inProgress, l:'Calling now', c:'#2563eb'},
              {n:answered,   l:'Answered',    c:'#16a34a'},
              {n:booked,     l:'Booked',      c:'#7c3aed'},
            ].map(({n,l,c})=>(
          ...
          {queue.length===0 && (
            <div style={{padding:'60px 20px', textAlign:'center', color:'#9ca3af'}}>
              <div style={{width:42, height:42, borderRadius:'50%', background:'#f4f4f2', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 12px', fontSize:18}}>📞</div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            {queue.map(item=>{
              ...
              const borderColor = active ? '#93c5fd' : item.status==='completed' ? '#86efac' : item.status==='failed' ? '#fca5a5' : '#e4e4e0'

              return (
                <div key={item.leadId}
                  style={{background:'#fff', border:`1px solid ${borderColor}`, borderRadius:11, padding:'11px 13px', display:'flex', alignItems:'center', gap:10, cursor:'pointer'}}
                ...
                >
                  ...
                  {item.status==='queued' && (
                    <button onClick={e=>{e.stopPropagation(); removeFromQueue(item.leadId)}} style={{fontSize:10, padding:'3px 7px', border:'1px solid #d1d5db', background:'#fff', borderRadius:6, cursor:'pointer', fontFamily:'inherit', color:'#6b7280'}}>Remove</button>
                  )}
                </div>
              )
            })}
        ...
        {selectedCall && (
          <div style={{background:'#fff', borderLeft:'1px solid #e4e4e0', overflowY:'auto', padding:18, display:'flex', flexDirection:'column', gap:14}}>
            ...
            <div>
              ...
              <div style={{display:'flex', flexWrap:'wrap', gap:4}}>
                {(selectedCall.lead.signals||[]).map(s=>{
                  const sig=SIGNALS[s]
                  const cmap:{r:string,a:string,b:string}={r:'#991b1b',a:'#78350f',b:'#1e3a8a'}
                  const bgmap:{r:string,a:string,b:string}={r:'#fee2e2',a:'#fef3c7',b:'#dbeafe'}
                  return <span key={s} style={{fontSize:10, padding:'2px 7px', borderRadius:99, fontWeight:600, background:bgmap[sig?.color||'r'], color:cmap[sig?.color||'r']}}>{sig?.label||s}</span>
                })}
              </div>
            </div>
            ...
            {selectedCall.error && (
              <div style={{background:'#fff5f5', border:'1px solid #fca5a5', borderRadius:8, padding:'10px 12px', fontSize:11, color:'#991b1b'}}>
